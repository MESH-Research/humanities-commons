 ...
 * Controller.
 */
class Humcore_Deposits_Keyword_REST_Controller extends WP_REST_Controller {
	...
	public function __construct() {
		...
		$this->namespace = 'humcore-deposits-keyword/v1';
	 ...
	 *
	 * humcore_deposits_keyword_list() already uses natcasesort(), this does additional sorting
	 *
	...
	public function get_terms( WP_REST_Request $data ) {
		...
		$cache_key = 'humcore_deposits_keyword_terms_' . sanitize_title( $user_input );

		$matched_terms = wp_cache_get( $cache_key );

		if ( ! $matched_terms ) {

			$all_terms = humcore_deposits_keyword_list();
