...
class Humanities_Commons {
	...
	public function __construct() {
		...
		//this is how you tap into the bp extended profile save action.
		add_action( 'bp_members_admin_load', array( $this, 'hcommons_add_member_type_save' ), 999, 1 );
		...
		add_filter( 'wpmn_get_taxonomy_term_site_id', array(
			$this,
			'hcommons_filter_hc_taxonomy_storage_site'
		), 10, 2 );
		...
		add_action( 'pre_user_query', array(
			&$this,
			'hcommons_filter_site_users_only'
		) ); // do_action_ref_array() is used for pre_user_query
		...
		add_filter( 'bp_before_activity_get_parse_args', array(
			$this,
			'hcommons_set_network_admin_activities_query'
		) );
		...
		add_action( 'bp_members_admin_user_metaboxes', array(
			$this,
			'hcommons_remove_member_type_meta_boxes'
		), 10, 2 );
		...
		add_filter( 'bp_blogs_format_activity_action_new_blog_post', array(
			$this,
			'hcommons_blogs_format_activity_new_blog_post'
		), 10, 2 );
		add_filter( 'bp_blogs_format_activity_action_new_blog_comment', array(
			$this,
			'hcommons_blogs_format_activity_new_blog_comment'
		), 10, 2 );
	...
	public function hcommons_admin_manage_group_memberships_view() {
        ...
        <label>
            <input type="radio" name="autopopulate"
                   value="Y" <?php echo ( $autopopulate_meta == 'Y' ) ? 'checked' : ''; ?> />Yes
        </label>
        <br/>
        <label>
            <input type="radio" name="autopopulate"
                   value="N" <?php echo ( $autopopulate_meta == 'N' ) ? 'checked' : ''; ?> />No
        </label>
	...
	/**
	 * Callback to removes the default buddy press member type metabox
	 * @return void
	 */
	public function hcommons_remove_bp_members_admin_member_type() {
		remove_meta_box( 'bp_members_admin_member_type', 'page', 'side' );
	}

	/**
	 * Captures and sanitizes the user_id from the bp extended profile page
	 * @return int
	 */
	...
	/**
	 * Callback for the member type metabox save function.
	 * @return void
	 */
	public function hcommons_add_member_type_save() {
		$user_id = $this->get_user_id();

		if ( empty( $_POST['bp-member-type-nonce'] ) || empty( $_POST['bp-members-profile-member-types'] ) ) {
			//this will trigger on admin page load. It can be safely ignored in that case.
			return;
		}
		check_admin_referer( 'bp-member-type-change-' . $user_id, 'bp-member-type-nonce' );
		$this->hcommons_add_member_type_update( $user_id, $_POST['bp-members-profile-member-types'] );

	}

	/**
	 * Log debugger to handle strings and arrays.
	 * Todo Should really be moved to a static utility function list/class.
	 *
	 * @param $log
	 *
	 * @return void
	...
	/**
	 * Handles the write function for bp member type metabox
	 *
	 * @param int $user_id
	 * @param array|null $membertypes
	 *
	 * @return void
	 */
	public function hcommons_add_member_type_update( $user_id = 0, array $membertypes = null ) {
		$user_id          = $user_id ?: $this->get_user_id();
		$post_memberships = ! empty( $_POST['bp-members-profile-member-types'] ) ? $_POST['bp-members-profile-member-types'] : null;
		$membertypes      = ! empty( $membertypes ) ? $membertypes : $post_memberships;

		if ( ! empty( $membertypes ) ) {
			bp_set_member_type( $user_id, "", false );
			foreach ( $membertypes as $member_type ) {
				$member_type = strtolower( $member_type );
				if ( ! bp_get_member_type_object( $member_type ) ) {
					$this->write_log( 'ERROR: bp_get_member_type_object did not find member type ' . $member_type );
					$this->write_log( 'Available member types:' );
					$this->write_log( bp_get_member_types( array(), 'objects' ) );
					continue;
				}
				bp_set_member_type( $user_id, $member_type, true );
			}
		} else {
			bp_set_member_type( $user_id, "HC", false );
		}
	}


	/**
	 * Outputs view for member type meta box on bp extended profile screen
	 *
	 * @return void
	 */
	public function hcommons_member_type_meta_box_view() {
		...
		if ( ! empty( $member_types ) ) {
			...
			$missing_hc               = empty( $search_member_type_array['hc'] ) ? true : false;
		}
		...
		if ( ! is_super_admin() ):
			...
			$membersTypeJson = strtolower( json_encode( bp_get_member_types() ) );
			//Todo maybe move function below to a js file.
			?>
            <script>
				<?php
				/*
                     * For possible future use. Todo edit to work with html below.
                     * https://jsfiddle.net/jeremykenedy/nfnc5ogh/
                     *
                     * function getAjaxDataList () {
                ...
                function write_member_type() {
                    const term = document.getElementById('new-membertype');
                    let value = term.value.toLowerCase();
                    const lis = document.getElementById("membertypelist").getElementsByTagName("li");
                    let isDupe = false;
                    Array.from(lis).forEach(function (child) {
                        let li_value = child.textContent.toLowerCase().replace(/[^a-zA-Z ]/g, "");
                        let button_value = "remove term " + value + value;
                        let test_a = li_value === button_value;
                        let test_b = li_value === value;
                        if (test_a || test_b) {
                            console.log('returning false');
                            //Already there, so return false to cancel the operation
                            isDupe = true;
                        }
                    });

                    const memberTypes = JSON.parse('<?php echo $membersTypeJson?>');
                    if (value in memberTypes && isDupe === false) {
                        value = value.toUpperCase();
                        const ul = document.getElementById('membertypelist');
                        const liElements = ul.getElementsByTagName("li");
                        const template = "<button onclick=\"this.parentNode.remove()\" type=\"button\" id=\"post_tag-check-num-" + liElements.length + "\" class=\"ntdelbutton\"><span class=\"remove-tag-icon\" aria-hidden=\"true\"></span><span class=\"screen-reader-text\">Remove term: " + value + "</span></button>&nbsp;" + value + "<input type='hidden' name='bp-members-profile-member-types[]' value='" + value + "'/>";
                        let li = document.createElement("li");
                        li.appendChild(document.createTextNode("hello"));
                        let newLiID = "member_type_li_" + liElements.length;
                        li.setAttribute("class", "member_type_li " + value + "_member_type_li");
                        li.setAttribute("id", newLiID);
                        ul.appendChild(li);
                        document.getElementById(newLiID).innerHTML = template;
                        term.value = "";
                    }
                }
            </script>
            <div class="tagsdiv" id="post_tag">
                <div class="jaxtag">
                    <div class="ajaxtag hide-if-no-js">
                        ...
                        <p><input type="text" id="new-membertype" list="membertypes"
                                  class="newtag form-input-tip ui-autocomplete-input" size="16" autocomplete="on"
                                  aria-describedby="new-tag-post_tag-desc" value="" role="combobox"
							...
							// Possible solution: https://github.com/thgreasi/datalist-polyfill
							?>
                            <datalist id="membertypes">
								<?php
								foreach ( json_decode( strtoupper( $membersTypeJson ) ) as $type ) { ?>
                                <option value="<?php echo $type ?>">
									<?php } ?>
                            </datalist>
                            <input type="button" onclick="write_member_type()" class="button tagadd" value="Add">
                        </p>
                    </div>
                </div>

                <ul id="membertypelist" class="tagchecklist" role="list">
					<?php
					$cnt = 0;
					$li  = "";

					if ( $missing_hc ) {
						$li .= "<li class='member_type_li " . "HC" . "_member_type_li' id='member_type_li_" . $cnt . "'>" . strtoupper( "HC" ) . "<input type='hidden' name='bp-members-profile-member-types[]' value='" . strtoupper( "HC" ) . "'/></li>";
						echo $li;
					}
					if ( ! empty( $member_types ) ) {
						if ( false !== $key = array_search( "hc", $member_types ) ) {
							$move = $member_types[ $key ];
							unset( $member_types[ $key ] );
							array_unshift( $member_types, $move );
						}

						foreach ( $member_types as $type ) {
							//User shouldn't be able to remove HC
							$button = strtoupper( $type ) === "HC" ? "" : "<button onclick=\"this.parentNode.remove()\" type=\"button\" id=\"post_tag-check-num-" . $cnt . "\" class=\"ntdelbutton\"><span class=\"remove-tag-icon\" aria-hidden=\"true\"></span><span class=\"screen-reader-text\">Remove term: " . strtoupper( $type ) . "</span></button>";
							$li     = "<li class='member_type_li " . $type . "_member_type_li' id='member_type_li_" . $cnt . "'>" . $button . "&nbsp;" . strtoupper( $type ) . "<input type='hidden' name='bp-members-profile-member-types[]' value='" . strtoupper( $type ) . "'/></li>";
							echo $li;
							$cnt ++;
						}
					}
