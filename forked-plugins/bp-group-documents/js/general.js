...
jQuery(document).ready( function($) {
	...
	$('#bp-group-documents-sort-form select[name=order]').on( "change", 
		function() {
			$('form#bp-group-documents-sort-form').trigger( "submit" );
		}
	);

	//Hide the category form submit, we're gonna submit on change
	$('#bp-group-documents-category-form input[type=submit]').hide();
	$('#bp-group-documents-category-form select[name=category]').on( "change",
		function() {
			$('form#bp-group-documents-category-form').trigger( "submit" );
		}
	);
	...
	$('#bp-group-documents-upload-button').on( "click",
		function() {
	...
	$('input.bp-group-documents-new-category')
		.val(l10nBpGrDocuments.new_category)
		.css('color','#999')
		.on( "focus", 
			function() {
	...
	$('form#bp-group-documents-form').on( "submit", 
		function()	{
	...
	$('form#group-settings-form').on( "submit", 
		function() {
			//check for pre-filled values, and remove before sumitting
			if( $('input.bp-group-documents-new-category').val() === l10nBpGrDocuments.new_category  ) {
				$('input.bp-group-documents-new-category').val('');
	...
	$('a.bp-group-documents-delete').on( "click",
		function()	{
	...
	$('#group-documents-group-admin-categories input[value=Add]').on( "click",
		function() {
			$.post(
				ajaxurl, 
				{
